...
#        fig1.savefig("timeSL.pdf",dpi = 600)
#
#
#        fig3 = pl.figure()
#        ax3 = fig3.add_subplot(111)
#        # just plot the last 10,000 pts
#        ax3.plot(sol[(totIter-10000):,2],sol[(totIter-10000):,0],label="Last Part Phase Space")
...
#        fig7.savefig("restricttimeSL.png")
        ...
        modtime = pl.arange(0,int(totIter*dt),dt)
        for y,z in enumerate(modtime):
            modtime[y] = z%(2*pl.pi)

        fig = pl.figure()
        ax = fig.gca(projection='3d')
        # just plot the last 10,000 pts
        ax.scatter(sol[(totIter-18000):,2],sol[(totIter-18000):,0],modtime[(totIter-18000):],label="Full Trajectory",s=.2)
        #ax.scatter([0.0,pl.pi,2*pl.pi],[0.0,0.0,0.0],color = "Red", marker="o",label="Electrodes")
        ax.set_title("Full Phase Space")
        ax.legend(loc = "best")
        ax.set_xlabel("Position")
        ax.set_ylabel("Velocity")
        ax.set_zlabel("Time")
        #pl.show()
        fig.savefig("phaseSpace.pdf",dvi = 300)
