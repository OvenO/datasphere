...
def main():
    ...
    # input wich parameter we are running the sweep over. default will be "coef" (A). i.e codimesion
    #parser.add_argument('--codim', action = 'store', dest = "codim",type = str,required = False)
    ...
    damping = float(lns[5])
    print('damping from file is: ' +str(damping))
    dt = float(lns[7])
    print('dt from file is: ' +str(dt))
    d = float(lns[3])
    print('d from file is: ' + str(d))
    ...
    print('coefficient from file is: '+str(coefficient))
    ...
    for i in xrange(p_num):
        ...
        apx = ec.TwinECApx(coefficient,damping,d,dt)
